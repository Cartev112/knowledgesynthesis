...
def _create_prompt(max_triplets: int = 50) -> str:
    return (
        ...
        "Extract BOTH explicit and implicit semantic relationships from scientific text, focusing on observations, definitions, lemmas, theorems, experimental results, and conclusions. "
        "\n\n"
        "Rules for extraction:\n"
        "1) Return ONLY a JSON object with a single field 'triplets' which is an array of objects.\n"
        "2) Each triplet object MUST include: subject, predicate, object, subject_types, object_types.\n"
        "   - subject_types and object_types must each be an array of strings (no empty strings)\n"
        "   - original_text is OPTIONAL: only include if 1-2 sentences succinctly convey the relationship\n"
        "3) CONCEPT TYPES: For subject_types and object_types, classify each concept based on its role in the text:\n"
        "   - Use descriptive, domain-appropriate type names (e.g., 'Metric', 'Framework', 'Process', 'Entity', 'Property', 'Outcome')\n"
        ...
        "   - Include multiple types when the text clearly indicates different facets \n"
        ...
        "   - Extract negative relationships when they are explicitly OR implicitly stated in the text\n"
        ...
        "9) Extract relationships that are clearly implied or synthesized from the text, not just those with direct quotes.\n"
